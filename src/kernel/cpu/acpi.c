...
#include "../include/cpu/acpi.h"
#include "../include/cpu/io.h"

#include "../include/utils/stdlib.h"

static const uint8_t SIG_RSDP[8] = { 'R', 'S', 'D', 'P', ' ', 'P', 'T', 'R', ' ' };
static const uint8_t SIG_RSDT[4] = { 'R', 'S', 'D', 'T' };
static const uint8_t SIG_XSDT[4] = { 'X', 'S', 'D', 'T' };
static const uint8_t SIG_MADT[4] = { 'M', 'A', 'D', 'T' };

#define IDMAP_LIMIT         0x1000000ULL
#define ACPI_MAX_TABLE_LEN  0x1000ULL
...
static inline void lapic_write(uint64_t base, uint32_t reg, uint32_t value) {
...
static inline uint32_t ioapic_read(uint64_t base, uint8_t reg) {
    *(volatile uint32_t *)((uint8_t *)base + IOREGSEL) = reg;
    return *(volatile uint32_t *)((uint8_t *)base + IOWIN);
}

static inline void ioapic_write(uint64_t base, uint8_t reg, uint32_t value) {
    *(volatile uint32_t *)((uint8_t *)base + IOREGSEL) = reg;
    *(volatile uint32_t *)((uint8_t *)base + IOWIN) = value;
}

static inline uint64_t apic_base_from_msr(void) {
    uint64_t value = rdmsr(IA32_APIC_BASE_MSR);
    if ((value & IA32_APIC_BASE_EN) == 0)
        return 0;

    return value & IA32_APIC_BASE_MASK;
}
...
static acpi_sdt_header_t *find_table_in_rsdt(acpi_sdt_header_t *rsdt, const uint8_t sig[4]) {
...
static acpi_sdt_header_t *find_table_in_xsdt(acpi_sdt_header_t *xsdt, const uint8_t sig[4]) {
...
static void parse_madt(madt_t *madt, apic_info_t *out) {
    ...
    while (p + sizeof(madt_entry_header_t) <= end) {
        ...
        if (eh->type == MADT_TYPE_LOCAL_APIC && eh->length >= sizeof(madt_local_apic_t)) {
        ...
        } else if (eh->type == MADT_TYPE_LOCAL_APIC_ADDR_OVERRIDE && eh->length >= sizeof(madt_lapic_addr_override_t)) {
            madt_lapic_addr_override_t *e = (madt_lapic_addr_override_t *)p;
            out->lapic_phys_addr = e->lapic_phys_addr;
        } else if (eh->type == MADT_TYPE_LOCAL_X2APIC && eh->length >= sizeof(madt_local_x2apic_t)) {
            ...
            if (out->cpu_count < ACPI_MAX_CPUS) {
                ...
                out->cpus[out->cpu_count].flags = (uint8_t)e->flags;
                ++out->cpu_count;
            }
        }
...
void apic_eoi(void) {
    ...
    lapic_write(lapic_base, LAPIC_EOI_REG, 0x00);
}
...
static void ioapic_route_irq(uint8_t irq, uint8_t vector) {
    ...
    ioapic_write(IOAPIC_PHYS, hi_reg, lo_reg);
